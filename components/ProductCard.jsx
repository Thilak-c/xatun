...
const ProductCard = ({ title, price, id, imageUrl }) => {
  ...
  return (
    <div className="mb-10 relative h-[310px] hover:scale-125 transition-all delay-150 max-w-sm w-[212px] bg-blue-800 bg-opacity-15 rounded-lg shadow-blue-700 shadow-xl overflow-hidden">
      <motion.div
      ...
      <div>
        ...
        <h2 className=""><samp className="text-green-500 mr-2 font-bold">₹</samp>{price} <samp className="text-2xl text-gray-600">.</samp> <samp className="line-through text-red-400">₹ {(1.2*price).toFixed(2)}</samp></h2>
        </div>
      </div>
    </div>
  );
